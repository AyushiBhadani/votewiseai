 ...
 * aiUtils.ts
 * 
 * Shared AI utility functions used primarily by the /api/chat route.
 * Provides intent detection, in-memory response caching, official 
 * registration URL lookups, and Gemini history format builders.
 * 
 * @module
 */
...
/** The recognized user intents for the AI routing logic. */
export type Intent =
  | 'greeting'
  | 'eligibility'
  | 'registration'
...
/** Represents a cached AI response entry with an expiration timestamp. */
export interface CacheEntry {
  value: string;
  expiresAt: number;
}

/** Standard internal representation of a chat message. */
...
/**
 * Detects the core intent of a user's message using RegExp pattern matching.
 * Checks patterns in a predefined priority order to ensure accurate matching.
 * 
 * @param message - The raw text message from the user.
 * @returns The detected Intent category (falls back to 'general').
 */
...
/**
 * Returns the official voter registration URL and label for a given country.
 * If the country is not mapped, returns a fallback Google search URL.
 * 
 * @param country - The name of the country.
 * @returns An object containing the target `url` and descriptive `label`.
 */
...
/**
 * Generates a deterministic cache key for storing and retrieving AI responses.
 * Based on the app's current context so identical prompts across different 
 * languages/countries do not clash.
 * 
 * @param message - The raw user message.
 * @param country - The active country context.
 * @param language - The active language context.
 * @param mode - The current chat mode ('chat' or 'story').
 * @returns A uniquely concatenated string key.
 */
 ...
 * 
 * @param key - The cache key generated by `buildCacheKey`.
 * @returns The cached response string, or null if missing/expired.
 */
...
/**
 * Stores an AI response in the in-memory cache with a 1-hour expiration.
 * 
 * @param key - The cache key generated by `buildCacheKey`.
 * @param value - The generated AI response to store.
 */
...
/**
 * Clears expired cache entries. Call periodically to prevent memory 
 * leaks in long-running Node.js server environments.
 */
...
/**
 * Converts the application's internal ChatMessage format into the structure
 * required by the Google Gemini multi-turn API.
 * Limits the history to the last `maxMessages` to keep prompt size manageable.
 * 
 * @param messages - Array of previous chat messages.
 * @param maxMessages - Maximum number of recent messages to include (default: 10).
 * @returns Array of formatted Content objects ready for Gemini's `startChat`.
 */
